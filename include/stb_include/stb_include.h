...
char* stb_include_string_preloaded(char* str, char* inject, std::span<const std::array<const char*, 2>> includeDict, const char* filename_for_line_directive, char error[256]);
...
static char* stb_include_file_preloaded(char* filename, char* inject, std::span<const std::array<const char*, 2>> includeDict, char error[256])
{
    char* result;
    char* text = NULL;
    for (const std::array<const char*, 2>& includeEntry : includeDict) {
        if (!strcmp(includeEntry[0], filename)) {
            text = strdup(includeEntry[1]);
            break;
        }
    }
    if (text == NULL) {
        strcpy(error, "Error: couldn't find '");
        strcat(error, filename);
        strcat(error, "'");
        return 0;
    }
    result = stb_include_string_preloaded(text, inject, includeDict, filename, error);
    free(text);
    return result;
}


char *stb_include_string_preloaded(char *str, char *inject, std::span<const std::array<const char*, 2>> includeDict, const char* filename, char error[256]) {
    char temp[4096];
    include_info* inc_list;
    int i, num = stb_include_find_includes(str, &inc_list);
    size_t source_len = strlen(str);
    ...
    for (i = 0; i < num; ++i) {
        ...
        if (inc_list[i].filename == 0) {
            ...
            char* inc = stb_include_file_preloaded(inc_list[i].filename, inject, includeDict, error);
            if (inc == NULL) {
                stb_include_free_includes(inc_list, num);
                return NULL;
            }
